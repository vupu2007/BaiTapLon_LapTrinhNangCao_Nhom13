...
public class AuctionDetailController implements Observer {
    ...
    public void initialize() {
        ...
        if (btnSubmitBid != null) btnSubmitBid.setOnAction(event -> handleManualBid());
        if (btnAutoBid != null) btnAutoBid.setOnAction(event -> handleAutoBidToggle());
    }
    ...
    public void update(double newPrice, String usernameAndTimeToParse) {
        ...
        Platform.runLater(() -> {
            ...
            if (isAutoBidEnabled && CurrentAccount.getAccount() != null
                    && !targetUser.equals(CurrentAccount.getAccount().getUsername())) {
                triggerAutoBidSystem(newPrice);
            }

            // CHẶN TRÙNG LOG
            if (newPrice == lastProcessedPrice) return;
                ...
                try {
                    if (!targetEndTimeStr.equals(this.lastEndTimeStr)) {
                        ...
                        if (vboxBidHistoryContainer != null) {
                            Label alertLog = new Label(String.format(
                                    "⚡ [%s] [GIA HẠN] Đặt giá giây cuối, phiên đấu giá tăng thêm 60 giây!",
                        ...
                        startCountdownClock(LocalDateTime.parse(targetEndTimeStr, dtf));
                    }
                } catch (Exception e) {
        ...
        countdownTimeline = new Timeline(new KeyFrame(Duration.seconds(1), event -> {
            ...
            if (duration.isZero() || duration.isNegative()) {
                ...
                if ("UPCOMING".equals(status) || "Sắp diễn ra".equalsIgnoreCase(status)) {
                    ...
                    if (lblTimeRemaining != null) {
                        lblTimeRemaining.setText("Đang chốt kết quả...");
                        lblTimeRemaining.setStyle("-fx-text-fill: #f39c12; -fx-font-weight: bold;");
                    }
    ...
    private void checkBiddingPermissions(int sellerId) {
        ...
        if (currentAuction != null && currentAuction.getStartTime() != null
                && LocalDateTime.now().isBefore(currentAuction.getStartTime())) {
    ...
    public void loadProductDetail(Item item) {
        ...
        // ✅ Hiển thị UI trước với "Đang tải..."
        Platform.runLater(() -> {
            fillTextFields(item.getName(), formattedPrice, item.getDescription(),
                    "Đang tải...", "--/--/---- --:--", "--/--/---- --:--");
        ...
        detailService.fetchAccountByIdAsync(item.getOwnerId(), seller -> {
            String sellerName = (seller != null) ? seller.getUsername() : "Người bán #" + item.getOwnerId();
    ...
    public void loadProductDetail(Auction auction) {
        ...
        boolean isEnded = "FINISHED".equalsIgnoreCase(status)
                || "SOLD".equalsIgnoreCase(status)
                || "Kết thúc".equalsIgnoreCase(status);
        ...
        if (isEnded) {
            ...
            boolean notStarted = auction.getStartTime() != null
                    && LocalDateTime.now().isBefore(auction.getStartTime());
            if (lblTimeRemainingTitle != null)
                lblTimeRemainingTitle.setText(notStarted ? "Bắt đầu sau" : "Thời gian còn lại");
        ...
        detailService.fetchItemByIdAsync(auction.getItemId(), finalItem -> {
            ...
            String pName = (auction.getProductName() != null && !auction.getProductName().isEmpty())
            ...
            String desc = (finalItem != null && finalItem.getDescription() != null)
                    ? finalItem.getDescription() : "";

            Platform.runLater(() -> {
                fillTextFields(pName, startPriceStr, desc, "Đang tải...", startTimeStr, endTimeStr);
            ...
            // ✅ Lấy seller qua service
            detailService.fetchAccountByIdAsync(auction.getSellerId(), seller -> {
                String sellerName = (seller != null)
                        ? seller.getUsername() : "Người bán #" + auction.getSellerId();
                if (lblSellerName != null) lblSellerName.setText(sellerName);
            });

            // ✅ Lấy winner qua service nếu có
                ...
                if (vboxBidHistoryContainer != null && bids != null) {
                    ...
                    for (BidTransaction bid : bids) {
                        Label label = new Label(String.format("• [%s] %s đặt %,.0f đ",
                                bid.getBidTime() != null
                                        ? bid.getBidTime().plusHours(7).format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"))
                                        : "--:--:--",
                                bid.getBidderUsername(), bid.getBidAmount()));
                        label.setStyle("-fx-font-size: 14px; -fx-padding: 5px;");
                        vboxBidHistoryContainer.getChildren().add(label);
    ...
    private void fillTextFields(String title, String price, String desc,
        ...
        if (currentItem != null) {
            ...
            if ("FINISHED".equalsIgnoreCase(trangThai)
                    || "SOLD".equalsIgnoreCase(trangThai)
                    || "Kết thúc".equalsIgnoreCase(trangThai)) {
                if (lblTimeRemaining != null) {
                    lblTimeRemaining.setText("00h 00m 00s");
                    lblTimeRemaining.setStyle("-fx-text-fill: #f39c12; -fx-font-weight: bold;");
                }
    ...
    private void handleManualBid() {
        if (txtBidAmount == null || txtBidAmount.getText().trim().isEmpty()) {
            showAlert("Thông báo", "Vui lòng điền số tiền hợp lệ!"); return;
        }
        if (currentAuction == null) {
            showAlert("Lỗi", "Phiên đấu giá chưa kích hoạt!"); return;
        }
        try {
    ...
    private void handleAutoBidToggle() {
        ...
        if (btnAutoBid.isSelected()) {
            ...
            dialog.setHeaderText("Hệ thống tự động nâng giá tối thiểu 10% giá khởi điểm khi có đối thủ vượt bạn.");
    ...
    private void triggerAutoBidSystem(double currentOpponentBid) {
        double stepPrice = (currentAuction != null) ? currentAuction.getStartPrice() * 0.10 : 50000.0;
        ...
        Platform.runLater(() -> {
            try {
                ...
                if (fxmlLocation == null) {
                    System.err.println("❌ Không tìm thấy FXML: " + homeFxmlPath); return;
                }
                ...
                System.err.println("❌ Lỗi quay lại trang chủ: " + e.getMessage());
            }
        });
    }
}